...
def character_ngram_fscore(hypothesis: str, reference: str, n: int = 2) -> float:
    """
    Computes the Character N-gram F-score between hypothesis
    and reference string.

    The text is split into character n-grams, then Precision,
    Recall, and F-score are calculated.

    :param hypothesis: The predicted string.
    :param reference: The ground truth string.
    :param n: Size of the character n-gram (default = 2 for bigrams).
    ...
    hyp_ngrams = get_ngrams(list(Graphemizer(hypothesis)), n)
    ref_ngrams = get_ngrams(list(Graphemizer(reference)), n)

    # Handle edge cases
    if len(ref_ngrams) == 0:
        return 0.0 if len(hyp_ngrams) > 0 else 1.0

    hyp_count = Counter(hyp_ngrams)
    ref_count = Counter(ref_ngrams)

    # Count matching n-grams
    overlap = sum((hyp_count & ref_count).values())
...
def BPC(hypothesis: str, reference: str) -> float:
    """
    Computes the Bits-per-Character (BPC) between hypothesis
    and reference string.

    Here, BPC is approximated using character-level matching:
    matched characters are assigned a higher probability,
    mismatched characters are assigned a lower probability.

    BPC = -(1/N) * sum(log2(P(c)))

    :param hypothesis: The predicted string.
    :param reference: The ground truth string.
    :return: The BPC value as a float.
    """
    ...
    for i in range(N):
        if i < len(hyp_graphemes) and hyp_graphemes[i] == ref_graphemes[i]:
            prob = 0.9  # high confidence for correct match
        else:
            prob = 0.1  # low confidence for mismatch

        total_log_prob += math.log2(prob)

    bpc = -(total_log_prob / N)
    return bpc